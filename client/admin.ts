import { Station } from "../node/types"
...
async function setCurrentKey() {
        $("#currentKey").text("Current Event: " + await postDataGeneral({ action: "getKey" }))
}

$('#save-settings').on('click', function () {
        if ($('#eventKey').val() !== "") {
                postDataAdmin({ action: 'changeKey', data: $('#eventKey').val()?.toString() || "" }, true)
                $('#eventKey').val("")
                setCurrentKey()
        }
})
...
function JSONCompareByNumberOfMatches(a: any, b: any) {
...
function JSONCompareByPriority(a: any, b: any) {
...
function JSONCompareByMatchNumber(a: any, b: any) {
        if (a.match < b.match) {
                return -1;
        }
        if (a.match > b.match) {
