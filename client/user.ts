import { UserGetData } from "../node/types";
...
function generateTable(userGet: UserGetData) {
...
async function getNextMatch(userGet: UserGetData) {
...
let teams: TeamGetRes | undefined = undefined;
getTeams().then((res) => {
        teams = res
})

function getTeamName(number: number | undefined) {
        return teams?.teamNames[teams.teamNumbers.findIndex((t) => t == number)] || "????"
}

async function reloadData() {
        const userGet = await fetch("/user-get/").then((res) => res.json()) as UserGetData
